...
func getDuelGradient(c1, c2, c3, c4 color) []color {
	result := make([]color, 256)
	for i := range result {
		pct := float32(i) / float32(255)
		if pct < 0.5 {
...
func makeNoise(pixels []byte, frequency, gain, lacunarity float32, octaves int) {
	...
	// gradient := getDuelGradient(color{0, 0, 175}, color{80, 60, 244}, color{12, 192, 75}, color{255, 255, 255})
...
func main() {
	...
	for {
		...
		mult := 1
		if keyState[sdl.SCANCODE_RSHIFT] != 0 || keyState[sdl.SCANCODE_LSHIFT] != 0 {
			mult = -1
		}
