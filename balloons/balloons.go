		...
		for x := 0; x < newWidth; x++ {
			...
			if screenX >= 0 && screenX < winWidth && screenY >= 0 && screenY < winHeight {
				...
				for i := 0; i < 4; i++ {
					...
					pixels[screenIndex] = uint8(blerp(c00, c10, c01, c11, tx, ty))
					screenIndex++
				}
			}
		}
...
func loadBaloons() []texture {
	...
	for i, bstr := range balloonStr {
		...
		balloonTextures[i] = texture{balloonPixels, w, h, w * 4, pos{float32(i * 60), float32(i * 60)}, float32(1 + i)}
	}
...
func main() {
	...
	window, err := sdl.CreateWindow("Testing SDL", sdl.WINDOWPOS_UNDEFINED,
	...
	cloudTexture := texture{cloudPixels, winWidth, winHeight, winWidth * 4, pos{0, 0}, 1}

	pixels := make([]byte, winWidth*winHeight*4)
	balloonTextures := loadBaloons()
	dir := 1
